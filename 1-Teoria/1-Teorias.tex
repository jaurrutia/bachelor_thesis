...
Para el estudio del esparcimiento por una partícula arbitraria inmersa en una matriz con índice de refracción $n_m$, se considera que ésta es iluminada por una onda plana, cuya dirección de propagación define la dirección $z$, es decir,
	\begin{equation}
	\vb{E}^i = (E_x^i \vu{e}_x + E_y^i \vu{e}_y)e^{i(k z - \omega t)},
	\label{eq:Exyi}
	\end{equation}
donde $k = 2\pi n_m /\lambda$ es el número de onda. En la fig.  \ref{fig:PlanoEsparcimiento} se muestra  una partícula iluminada por una onda plana [Ec. \eqref{eq:Exyi}]; el origen del sistema coordenado se posiciona en el centro de la partícula y la dirección $z$ se alinea con la dirección de propagación de la onda plana, como se indica con el eje cartesiano $(x, y, z)$, así como las bases de vectores ortonormales retangulares $\{\vu{e}_x,\vu{e}_y,\vu{e}_z \}$, mostradas en azul, y polares $\{\vu{e}_r, \vu{e}_\theta, \vu{e}_\varphi \}$, mostrados en negro. De forma análoga al plano de incidencia\footnote{En el problema de un haz de luz incidente a una superficie plana, el plano de incidencia se define por el vector normal a la superficie y la dirección de propagación del haz.} se define el \emph{plano de esparcimiento}, en color verde en la Fig. \ref{fig:PlanoEsparcimiento}, con el vector de dirección del esparcimiento $\vu{e}_r$ y la dirección del haz incidente $\vu{e}_z$. Con base en el plano de esparcimiento se define la dirección ortogonal $\perp$ y paralela $\parallel$ de polarización.
...
\vb{E}_i &= [(\cos\varphi E_{x}^i + \sin\varphi E_{y}^i)\vu{e}_{\perp}^i +
		...
		\label{eq:EIncidente}
\end{align}
en donde se omite el término de la fase temporal $e^{-i\omega t}$ y la fase espacial $e^{ikz}$ se incluye en los coficientes $E_\perp^i$ y $E_\parallel^i$. Adicionalmente, para el campo eléctrico esparcido se emplea el campo debido a una fuente oscilante en el tiempo, por lo que el campo eléctrico es \cite{jackson1999electrodynamics}
	\begin{equation}
\vb{E}  = 	\frac{e^{ikr}}{4\pi\varepsilon_0}   \left[ \underbrace{\frac{k^2}{r} (\vu{e}_r\times\vb{p})\times \vu{e}_r}_{\text{transversal}}
		 +  \underbrace{[3\vu{e}_r(\vu{e}_r\cdot\vb{p})-\vb{p}] \left(\frac{1}{r^3}-\frac{ik}{r^2}\right) }_{\text{radial}} \right],
		 \label{eq:ELejanoFull}
	\end{equation}
en donde $\vb{p}$ es el momento dipolar. Si de la Ec. \eqref{eq:ELejanoFull} se consideran únicamente los términos que corresponden al campo lejano, es decir, el término con componentes transversales y que decae como $r^{-1}$ y cumple con la relación $k\ll 1$, el campo esparcido $\vb{E}^s$ puede escribirse como \cite{bohren1998absorption}
\begin{align}
\vb{E}^s &\propto \frac{e^{ikr}}{-ikr}\vb{E}_0^s \notag \\
		&=  \frac{e^{ikr}}{-ikr}
			\qty( E_{\perp}^s  \vu{e}_{\perp}^s + E_{\parallel}^s \vu{e}_{\parallel}^s) \label{eq:ELejano}
\end{align}
en donde  $\vb{E}_0^s$ es la amplitud del campo esparcido,  $ E_{\perp}^s$ y  $ E_{\parallel}^s$ sus componentes en la base de los vectores paralelo y perpendicular al plano de esparcimiento, y donde se cumple la propiedad $\vb{E}_0^s\cdot\vu{e}_r = 0$,. De esta forma,  es posible relacionar al campos eléctrico esparcido $\vb{E}^s$ [Ec. \eqref{eq:ELejano}] con el campo eléctrico  incidente $\vb{E}^i$ [Ec. \eqref{eq:EIncidente}  mediante la siguiente relación 
	\begin{equation}
	...
	\end{equation}
en donde la matriz  $\mathbb{S}$ es la matriz de esparcimiento y ,de forma general, $S_j = S_j(\theta,\varphi)$. Las componentes de la matriz de esparcimiento en la Ec. \eqref{eq:MEsparcimientoGral} dependen de la geometría de la partícula iluminada por la onda plano, por lo que al considerar una partícula esférica, se descompone a la onda plana  en una base ortonormal esférica para calcular a los campos esparcidos mediante las condiciones a la frontera de los campos EMs, y así, determinar la forma de la matriz de esparcimiemnto. En la siguiente sección se construye la base ortonormal esférica de los armónicos esféricos vectoriales mediante, la cual se emplea en la solución de Mie para el cálculo de los campos EMs.
...
\noindent	
en donde $k = n k_0$ es la magnitud del vector de onda y $n$ es el índice de refracción del material [Ec. \eqref{eq:indice}].
...
Cuando se considera una partícula esférica de radio $a$ e índice de refracción $n_p$, inmersa en un medio, denominado matriz, con índice de refracción $n_m$, como se observa en la Fig. \ref{fig:EsferaA}, y que ésta es iluminada por una onda plana propagándose a lo largo del eje $z$, se emplean coordenadas esféricas $\{ r, \theta, \varphi\}$. La la función generadora de los armónicos vectoriales en coordenadas esféricas es 
	\begin{equation}
...
Para resolver la Ec. \eqref{eq:AEV_psi} se asume que $\psi$ es el producto de tres funciones y cada una de éstas depende únicamente en una variable, es decir,
	...
	\end{equation}
Al sustituir la Ec. \eqref{eq:psiEsf} en la Ec. \eqref{eq:AEV_psi},  multiplicar por $r^2/R\Theta\Phi$ y despejar los términos que dependen únicamente de $r$, se obtiene como resultado que una función con dependencia radial es igual a una función con dependencia angular, por lo tanto se igualan a una constante $\ell (\ell +1)$
	\begin{equation}
...
en donde $J_\ell$ y $Y_\ell$ son las \emph{funciones de Bessel del primer y segundo tipo} respectivamente y $h_\ell$ son las \emph{funciones esféricas de Bessel del tercer tipo}, también denominadas como \emph{funciones esféricas de Hankel}. Todas las funciones esféricas de Bessel $z_\ell$ ---donde $z_\ell$ es cualquier función de las Ecs. \eqref{eqs:SphBessel}--- cumplen con las  relaciones de recurrencia
	...
	\end{align}
en donde los primeros dos órdenes ($\ell=0,1$) de $j_\ell$ son $j_0(\rho) = \sin\rho / \rho$ y $j_1(\rho) = \sin\rho / \rho^2- \cos\rho/\rho$, mientras que los de $y_\ell$ son $y_0(\rho) = -\cos\rho/\rho$ y $y_1(\rho) = -\cos\rho/\rho^2-\sin\rho/\rho$. Por lo tanto, las funciones generadoras de los armónicos esféricos vectoriales son
...
%
	...
	\end{tcolorbox}\vspace{-.5em}\noindent
en donde se definió el producto interior como $
\langle \vb{A},\vb{B} \rangle_{\theta,\varphi}$ como 
	\begin{align*}
...
Para los campos esparcidos ($\vb{E}^s,\,\vb{H}^s$) las funciones $j_\ell$ y $y_\ell$ no tienen puntos indeterminados, por lo que se emplearan las funciones esféricas de Hankel $h_\ell^{(1)}$ y $h_\ell^{(2)}$,. En su límite asintótico ($\ell^2\ll kr$), las funciones de esféricas de Hankel son \cite{bohren1998absorption}
...
por lo que $h_\ell^{(1)}$ corresponde a una onda esférica saliente, y $h_\ell^{(2)}$ una entrante. Dado que el campo esparcido es una onda saliente, se emplea $h_\ell^{(1)}$ como solución radial a la función generadora de los armónicos esféricos vectoriales. Entonces, los campos EMs esparcidos $(\vb{E}^s,\vb{H}^s)$ son 
	\begin{subequations}
	\eqhalf{\vb{E}^s = \sum_{\ell =1}^\infty E_\ell \qty(i a_\ell \vb{N}_{oe\ell}^{(3)}- b_\ell\vb{M}_{o1\ell}^{(3)}),
		\label{eqs:EsAEV}}
...
\noindent
en donde se denota mediante el superíndice $(3)$ que se emplea $h_\ell^{(1)}$ para la solción radial. Dado que para los campos EMs de la onda plana incidente y de los campos EMs esparcidos $m= 1$, se definen las funciones   $\pi_\ell$ y $\tau_\ell$ como  
	\begin{align*}
	...
	\end{align*}
para expresar la dependencia angular polar en los armónicos esféricos vectoriales [Ec. \eqref{eq:AEV}. Las relaciones de recurrencia de 
las funciones asociadas de Legendre \cite{arfken2001methods} permiten expresar a  $\pi_\ell$ y $\tau_\ell$ como  \cite{bohren1998absorption}
	\begin{align*}
	\pi_\ell(\mu) = \frac{2\ell-1}{\ell-1} \mu \pi_{\ell-1}(\mu) - \frac{\ell}{\ell-1}\pi_{\ell-2}(\mu)
	\hspace{1cm}\mbox{y}\hspace{1cm}
	\tau_\ell(\mu) = \ell \mu\pi_\ell(\mu) - (\ell+1)\pi_{\ell-1}(\mu),
...
en donde se empleó el cambio de variable $\mu = \cos\theta$ y se define   $\pi_0 =0 $ y $\pi_1 = 1$.  Las funciones $\pi_\ell$ y $\tau_\ell$ son funciones pares e impares, respectivamente, y a pesar de no ser orotogonales, sí lo son la suma aritmética de ellas, es decir
	...
	\label{eq:ortTauPi}
	\end{equation}
Empleando las funciones $\tau_\ell$ y $\pi_ell$, los armónicos esféricos son
	\begin{align*}
	\vb{M}_{o1\ell} =& \cos\varphi\pi_\ell(\cos\theta)z_\ell(kr)\vu{e}_\theta 
					- \sin\varphi\tau_\ell(\cos\theta)z_\ell(kr)\vu{e}_\varphi,\\ 
	\vb{M}_{e1\ell} =& -\sin\varphi\pi_\ell(\cos\theta)z_\ell(kr)\vu{e}_\theta 
					- \cos\varphi\tau_\ell(\cos\theta)z_\ell(kr)\vu{e}_\varphi,\\ 
	\vb{N}_{o1\ell} =& \sin\varphi \ell(\ell+1)\sin\theta\pi_\ell(\cos\theta)
						\frac{z_\ell(kr)}{kr}\vu{e}_r
					+ \sin\varphi\tau_\ell(\cos\theta)
						\frac{\qty[(kr) z_\ell(kr)]'}{kr}	\vu{e}_\theta,\\
					&+ \cos\varphi \pi_\ell(\cos\theta)
						\frac{\qty[(kr) z_\ell(kr)]'}{kr}	\vu{e}_\varphi,\\	
	\vb{N}_{e1\ell} =& \cos\varphi \ell(\ell+1)\sin\theta\pi_\ell(\cos\theta)
						\frac{z_\ell(kr)}{kr}\vu{e}_r
					+ \cos\varphi\tau_\ell(\cos\theta)
						\frac{\qty[(kr) z_\ell(kr)]'}{kr}	\vu{e}_\theta,\\
					&- \sin\varphi \pi_\ell(\cos\theta)
						\frac{\qty[(kr) z_\ell(kr)]'}{kr}	\vu{e}_\varphi,
\end{align*}
en donde se emplea el superíndce $(1)$ para $z_\ell = j_\ell$ y $(3)$ para $z_\ell = h_\ell^{(1)}$.
...
Para determinar los coeficientes $a_\ell,b_\ell,c_\ell$ y $d_\ell$ de las Ecs. \eqref{eq:EHpAEV} y \eqref{eq:EHsAEV} se emplean las condiciones a la frontera [Ec. \eqref{eq:CFEsfera}], por lo que se deben de satisfaces las ecuaciones
	...
	\eqhalf{H_\varphi^i + H_\varphi^s = H_\varphi^p,}	\vspace{-1em}\noindent

en $r =a$, que es la superficie de la partícula esférica. Al emplear la ortogonalidad de las funciones $\sin\varphi$ y $\cos\varphi$[Ec. \eqref{eq:ortTauPi}], la ortogonalidad de $\tau_\ell\pm\pi_\ell$ [Ec. \eqref{eq:ortTauPi}] y las expresiones de los campos EMs de la onda plana incidente [Ecs. \eqref{eq:EHiAEV}], de los campos EMs dentro de la partícula [Ecs. \eqref{eq:EHpAEV}] y los campos EMs esparcidos [Ecs. \eqref{eq:EHsAEV}] se obtienen el sistema de ecuaciones
	\begin{align}
	j_\ell(Nx)c_\ell + h_\ell^{(1)}(x)b_\ell &= j_\ell(x),\\
	\mu_m\qty[Nj_\ell(Nx)]'c_\ell + \mu_p\qty[xh_\ell^{(1)}(x)]'b_\ell &= \mu_p\qty[xj_\ell(x)]',\\
	\mu_m N j_\ell(Nx)d_\ell +\mu_p h_\ell^{(1)}(x)a_\ell &=\mu_p j_\ell(x),\\
	\qty[Nj_\ell(Nx)]'d_\ell + N\qty[xh_\ell^{(1)}(x)]'a_\ell &= N\qty[xj_\ell(x)]',
	\end{align}
en donde $'$ es la operación derivada respecto al argumento de las funciones de Bessel, $x = k_m a = 2 \pi n_m a /\lambda$ es el parámetro de tamaño y $N = n_p / n_m$ es el índice de refracción relativo entre la partícula y la matriz.
